...
/* Push an arbitrary duk_tval to the stack, and coerce it to a string using
 * ES6 ToPropertyKey() (http://www.ecma-international.org/ecma-262/6.0/#sec-topropertykey).
 * Also check if it's a valid array index and return that (or DUK__NO_ARRAY_INDEX
 * if not).
 */
DUK_LOCAL duk_uint32_t duk__push_tval_to_property_key(duk_context *ctx, duk_tval *tv, duk_hstring **out_h) {
	...
	/* FIXME: The revised ES6 ToPropertyKey() handling (ES5.1 was just
	...
	/* FIXME: remove this and use DUK_TVAL_SET_STRING() in the fast path. */
	...
	if (DUK_TVAL_IS_STRING(tv)) {
		...
		h = duk_to_property_key_hstring(ctx, -1);
	}
	DUK_ASSERT(h != NULL);
	*out_h = h;
...
DUK_INTERNAL duk_hstring *duk_hobject_get_internal_value_string(duk_heap *heap, duk_hobject *obj) {
	...
	if (duk_hobject_get_internal_value(heap, obj, &tv)) {
		...
		h = DUK_TVAL_GET_STRING(&tv);
		/* FIXME: no check for symbol, should be OK */
		return h;
	}
...
DUK_INTERNAL duk_bool_t duk_hobject_getprop(duk_hthread *thr, duk_tval *tv_obj, duk_tval *tv_key) {
	...
	case DUK_TAG_STRING: {
		...
		if (DUK_HSTRING_HAS_ES6SYMBOL(h)) {
			/* Symbols don't have virtual properties. */
...
DUK_INTERNAL duk_bool_t duk_hobject_delprop(duk_hthread *thr, duk_tval *tv_obj, duk_tval *tv_key, duk_bool_t throw_flag) {
	...
	if (DUK_TVAL_IS_OBJECT(tv_obj)) {
		...
		key = duk_to_hstring(ctx, -1);  /* FIXME: symbol */
		DUK_ASSERT(key != NULL);
...
DUK_INTERNAL duk_bool_t duk_hobject_object_ownprop_helper(duk_context *ctx, duk_small_uint_t required_desc_flags) {
	...
	h_v = duk_to_hstring(ctx, 0);  /* FIXME: accept symbol */
	DUK_ASSERT(h_v != NULL);
